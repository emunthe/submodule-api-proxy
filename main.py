...
@app.get(
    "/stored_pre_cache_data",
    summary="Get stored precache data",
    description="Get all stored precache data including valid seasons, tournaments, matches, and team IDs",
    tags=["Cache"],
)
async def get_stored_pre_cache_data():
    """Get all stored precache data from Redis."""
    redis_client = None
    try:
        redis_client = get_redis_client()
        
        # List of precache data keys
        precache_keys = [
            "valid_seasons",
        ...
        # Get data for each precache key
            ...
            if raw_data:
                try:
                    ...
                    result[key] = {
                        ...
                        "count": len(parsed_data.get("data", [])) if isinstance(parsed_data.get("data"), list) else "N/A"
                    }
        ...
        result["summary"] = {
            "total_precache_keys": len(precache_keys),
            "keys_with_data": len([k for k, v in result.items() if k != "summary" and v.get("data") is not None]),
            "last_generated": pendulum.now().isoformat()
        }
        
        return result
        
    except Exception as e:
        logger.error(f"Error retrieving precache data: {e}")
        return {
            "error": f"Failed to retrieve precache data: {str(e)}",
            "status": "error"
        }
    finally:
        if redis_client:
